    ...
    <>
      ...
      <aside className="hidden w-60 shrink-0 lg:block">
        <div className="sticky top-24 rounded-2xl border border-gold/25 bg-night-soft p-3">
          <FilterList groups={groups} active={active} total={total} lang={lang} />
        </div>
      </aside>
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label={t.products.filterAria}>
          ...
          <div className="absolute inset-y-0 left-0 w-72 max-w-[85vw] overflow-y-auto border-r border-gold/25 bg-night-soft p-4">
            <div className="mb-2 flex items-center justify-between">
              <span className="font-heading text-lg font-semibold text-gold">{t.products.filter}</span>
              <button
                type="button"
                onClick={() => setOpen(false)}
                aria-label={t.products.closeFilter}
            ...
            <FilterList groups={groups} active={active} total={total} lang={lang} onNavigate={() => setOpen(false)} />
          </div>
        </div>
      )}
    </>
